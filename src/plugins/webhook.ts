...
export default fp<SupportPluginOptions>(async (fastify, opts) => {
    const webhook = new WebHook(opts?.DISCORD_WEBHOOK ?? process.env.DISCORD_WEBHOOK!)
    const exlude_fields :Array<string>= (opts?.exlude_fields|| []).concat(process.env?.WEBHOOK_EXCLUDE_FIELDS ? process.env.WEBHOOK_EXCLUDE_FIELDS.split(',') :[]).filter(x =>x?.length)
    fastify.decorate('webhook', webhook);
    fastify.decorate('webhook_exlude_fields', exlude_fields);
    fastify.addHook('onRequest', async function (_req: FastifyRequest<IWebhookRequest>, _reply: FastifyReply) {
        ...
        _req.sendMessagetoWebhook = async function (msg?: SendDiscordWebhookMessage) {
            return _req.server?.webhook?.sendMessage(msg ? msg : await _req.createWebhookMessage())
        }
